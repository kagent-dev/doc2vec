...
import { EmbeddingProviderFactory } from './embedding-factory';
...
class Doc2Vec {
    ...
    constructor(configPath: string) {
        ...
        // Initialize embedding provider based on environment variables
        this.embeddingProvider = EmbeddingProviderFactory.createProvider(this.logger);
        this.logger.info(`Using embedding provider: ${this.embeddingProvider.getProviderName()}`);
    ...
    private async createEmbeddings(texts: string[]): Promise<number[][]> {
        return await this.embeddingProvider.createEmbeddings(texts);
    }
}
